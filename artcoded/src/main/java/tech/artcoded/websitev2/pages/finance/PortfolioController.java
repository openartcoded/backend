...
public class PortfolioController {
    ...
    @DeleteMapping("/tick")
    public ResponseEntity<Map.Entry<String, String>> deleteTickFromPortfolio(@RequestParam("id") String id,
            @RequestParam("symbol") String symbol) {
        this.portfolioRepository.findById(id)
                .ifPresent(
                        portfolio -> portfolioRepository.save(portfolio
                                .toBuilder().ticks(portfolio.getTicks().stream()
                                        .filter(t -> !t.getSymbol().equals(symbol)).collect(Collectors.toSet()))
                                .build()));
    ...
    public ResponseEntity<Void> updateTag(@RequestBody Set<Tick> ticks, @RequestParam("id") String portfolioId) {
        portfolioRepository.findById(portfolioId).map(p -> p.toBuilder().ticks(ticks).updatedDate(new Date()).build())
    ...
    public Portfolio save(@RequestBody Portfolio portfolio) {
        Portfolio build = Optional.ofNullable(portfolio.getId()).flatMap(this.portfolioRepository::findById)
                .map(Portfolio::toBuilder).orElseGet(portfolio::toBuilder).name(portfolio.getName())
                .principal(portfolio.isPrincipal()).build();
        Portfolio portfolioSaved = portfolioRepository.save(build);
        if (portfolioSaved.isPrincipal()) {
            this.portfolioRepository.findByPrincipalIsTrue().stream()
                    .filter(p -> !p.getId().equals(portfolioSaved.getId()))
                    .map(p -> p.toBuilder().principal(false).build()).forEach(portfolioRepository::save);
        }
