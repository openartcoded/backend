...
public class InvoicePrimarySequenceSetAction implements Action {
    ...
    public ActionResult run(List<ActionParameter> parameters) {
        ...
        try {
            Long seq = parameters.stream().filter(p -> ACTION_PARAMETER_NUMBER_TO_SET.equals(p.getKey()))
                    .filter(p -> StringUtils.isNotEmpty(p.getValue())).findFirst()
                    .flatMap(p -> p.getParameterType().castLong(p.getValue())).orElse(0L);
    ...
    public static ActionMetadata getDefaultMetadata() {
        return ActionMetadata.builder().key(ACTION_KEY).title("Invoice Seq set")
                .description("An action to reset the sequence every year.")
                .allowedParameters(List.of(ActionParameter.builder().key(ACTION_PARAMETER_NUMBER_TO_SET)
                        .description("Number to set. Default is 0").parameterType(ActionParameterType.LONG)
                        .required(false).build()))
                .defaultCronValue("0 0 0 1 1 *").build();
    }
