...
public class MemZaGramService implements ILinkable {
    ...
    public void save(String id, String title, String description, Boolean visible, Date dateOfVisibility,
        ...
        log.info("getting thumbnail in a few secs...");

        TimeUnit.SECONDS.sleep(60);

        String thumbnailUploadId = ofNullable(imageUploadId).flatMap(this.fileUploadService::findOneById)
                .map(FileUpload::getThumbnailId).orElse(null);

        log.info("getting {} thumbnail id", thumbnailUploadId);
