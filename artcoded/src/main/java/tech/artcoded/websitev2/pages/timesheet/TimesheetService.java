...
public class TimesheetService implements ILinkable {
    ...
    public Map<Integer, List<Timesheet>> findAllGroupedByYearFilterClientDisabled() {
        var clientsDisabled = this.billableClientService.findByContractStatus(ContractStatus.DONE).stream()
                .map(BillableClient::getName).toList();

        return this.repository.findAll().stream().filter(ts -> !clientsDisabled.contains(ts.getClientNameOrNA()))
    ...
    public Map<Integer, Map<String, List<Timesheet>>> findAllGroupedByYearAndClientName() {
        return this.findAllGroupedByYear().entrySet().stream()
    ...
    public Map<Integer, Map<String, List<Timesheet>>> findAllGroupedByYearAndClientNameFilterClientDisabled() {
        return this.findAllGroupedByYearFilterClientDisabled().entrySet().stream()
                .map(e -> Map.entry(e.getKey(),
                        e.getValue().stream().collect(Collectors.groupingBy(Timesheet::getClientNameOrNA))))
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }
    ...
    public Timesheet saveOrUpdateTimesheet(Timesheet timesheet) {
        ...
        return repository
                .save(timesheetFromDb.toBuilder().name(timesheet.getName()).periods(timesheet.getPeriods()).build());
    }
    ...
    public TimesheetPeriod saveOrUpdateTimesheetPeriod(String id, TimesheetPeriod timesheetPeriod) {
        ...
        TimesheetPeriod newPeriod = ts.getPeriods().stream().filter(t -> t.getId().equals(timesheetPeriod.getId()))
                .map(TimesheetPeriod::toBuilder).findFirst().orElseGet(timesheetPeriod::toBuilder)
                .shortDescription(timesheetPeriod.getShortDescription())
                .afternoonEndTime(timesheetPeriod.getAfternoonEndTime()).projectName(timesheetPeriod.getProjectName())
                .periodType(timesheetPeriod.getPeriodType()).date(timesheetPeriod.getDate())
                .afternoonStartTime(timesheetPeriod.getAfternoonStartTime())
                .morningEndTime(timesheetPeriod.getMorningEndTime())
                .morningStartTime(timesheetPeriod.getMorningStartTime()).build();

        Timesheet updatedTimesheet = ts.toBuilder()
                .periods(Stream
                        .concat(ts.getPeriods().stream().filter(p -> !p.getId().equals(newPeriod.getId())),
                                Stream.of(newPeriod))
                        .sorted(Comparator.comparing(TimesheetPeriod::getDate)).collect(Collectors.toList()))
                .build();
    ...
    protected Timesheet defaultTimesheet(String clientId) {
        ...
        return Timesheet.builder().name(DateTimeFormatter.ofPattern("MM/yyyy").format(LocalDate.now()))
                .yearMonth(YearMonth.now()).clientId(client.getId()).clientName(client.getName())
                .student(client.isStudent())
                .settings(TimesheetSettings.builder().minHoursPerDay(BigDecimal.ZERO)
                        .maxHoursPerDay(new BigDecimal("8.5")).defaultProjectName(client.getProjectName()).build())
    ...
    public Timesheet generateInvoiceFromTimesheet(String id) {
        ...
        if (timesheet.isStudent()) {
            throw new RuntimeException("student timesheet, cannot generate invoice");
        }
        ...
        var template = invoiceService.listTemplates().stream()
                .sorted((i1, i2) -> i2.getDateCreation().compareTo(i1.getDateCreation())).findFirst()
                .orElseThrow(() -> new RuntimeException("missing invoice template. at least one needed"));
    ...
    public BigDecimal estimateTotalToBeInvoicedThisMonth() {
        ...
        for (var ts : timesheets) {
            ...
            var client = billableClientService.findById(ts.getClientId())
                    .orElseThrow(() -> new RuntimeException("client not found %s".formatted(ts.getClientId())));
    ...
    public void closeTimesheet(String id) {
        ...
        String uploadId = fileUploadService.upload(MockMultipartFile.builder().name("timesheet-" + id + ".pdf")
                .originalFilename("timesheet-" + id + ".pdf").contentType(MediaType.APPLICATION_PDF_VALUE).bytes(bytes)
                .build(), timesheet.getId(), false);
        var saved = repository.save(timesheet.toBuilder().closed(true).uploadId(uploadId).build());
        this.eventService.sendEvent(TimesheetReleased.builder().uploadId(uploadId).timesheetId(saved.getId())
                .clientName(saved.getClientNameOrNA()).period(saved.getName()).build());
        this.notificationService.sendEvent("New Timesheet Ready (%s)".formatted(saved.getName()), CLOSED_TIMESHEET,
                saved.getId());
    }
    ...
    public void reopenTimesheet(String id) {
        ...
        var saved = repository.save(timesheet.toBuilder().closed(false).invoiceId(null).uploadId(null).build());
        this.eventService.sendEvent(TimesheetReopened.builder().timesheetId(saved.getId())
                .clientName(saved.getClientNameOrNA()).period(saved.getName()).build());
        this.notificationService.sendEvent("Timesheet Reopened (%s)".formatted(saved.getName()), REOPENED_TIMESHEET,
                saved.getId());
    }
    ...
    public void deleteById(String id) {
        findById(id).ifPresent(ts -> {
            if (!ts.isClosed()) {
                ...
                this.eventService.sendEvent(TimesheetDeleted.builder().timesheetId(ts.getId())
                        .clientName(ts.getClientNameOrNA()).period(ts.getName()).build());
    ...
    public Timesheet updateSettings(TimesheetSettingsForm settings) {
        ...
        return repository.save(timesheet.toBuilder()
                .settings(timesheet.getSettings().toBuilder().defaultProjectName(client.getProjectName())
                        .maxHoursPerDay(settings.getMaxHoursPerDay()).minHoursPerDay(settings.getMinHoursPerDay())
                        .build())
                .clientName(client.getName()).clientId(client.getId()).build());
    }
