...
public class PortfolioNotifyAction implements Action {
    ...
    public ActionResult run(List<ActionParameter> parameters) {
        ...
        try {
            ...
            Long deltaValue = parameters.stream().filter(p -> PARAMETER_PERCENTAGE_DELTA.equals(p.getKey()))
            ...
            portfolioRepository.findAll().stream().flatMap(p -> p.getTicks().stream()).distinct()
                    .peek(tick -> messages.add("search for tick " + tick.getSymbol())).forEach(tick -> {
                        try {
                            var response = httpclient.send(
                                    HttpRequest.newBuilder().GET()
                                            .uri(URI.create("%s/%s?interval=1d&range=1d&region=EU"
                                                    .formatted(yahooFinanceChartUri, tick.getSymbol())))
                                            .build(),
                                    HttpResponse.BodyHandlers.ofString());
                            Double currentPrice = JsonPath.read(response.body(),
                            ...
                            String deltaMessage = "%s: %s%s".formatted(tick.getSymbol(),
                                    delta.setScale(2, RoundingMode.HALF_DOWN).toString(), "%");
    ...
    public ActionMetadata getMetadata() {
        return ActionMetadata.builder().key(ACTION_KEY).title("Portfolio Notify Action").description(
                "An action to notify when a tick (e.g TSLA) under perform to a defined percentage (default: 25%)")
                .allowedParameters(List.of(ActionParameter.builder().key(PARAMETER_PERCENTAGE_DELTA)
                        .parameterType(ActionParameterType.LONG).required(false)
                        .description(
                                "Represent a percentage delta that triggers the notification. Between 1-100. Default is 25")
                        .build()))
