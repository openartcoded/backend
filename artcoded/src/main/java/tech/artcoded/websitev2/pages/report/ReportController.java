...
public class ReportController {
    ...
    public Post newPost(Principal principal) {
        ...
        var post = Post.builder().status(PostStatus.IN_PROGRESS).id(IdGenerators.get()).priority(Priority.MEDIUM)
                .author(user.getEmail()).channelId(channelService.createChannel(id).getId()).title("Draft")
                .content("Content here").build();
    ...
    public ResponseEntity<Post> save(@RequestParam(value = "id") String id, @RequestParam("title") String title,
            ...
            @RequestParam("author") String author, @RequestParam(value = "status") PostStatus status,
        ...
        Post save = repository
                .save(post.toBuilder().tags(tags).priority(priority).author(author).title(title).content(content)
