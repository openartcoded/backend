...
public class ReportController {
  ...
  public ResponseEntity<Post> save(@RequestParam(value = "id") String id, @RequestParam("title") String title,
    ...
    Post save = repository
        .save(post.toBuilder().tags(tags).author(author).title(title).content(content).description(description)
  ...
  public void resetPostCount(@PathVariable("id") String id) {
    repository.findById(id).map(post -> repository.save(post.toBuilder().countViews(0).build()))
