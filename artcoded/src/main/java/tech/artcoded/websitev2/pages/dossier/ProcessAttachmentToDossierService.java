  ...
  public Dossier processFees(List<Fee> fees, Dossier dossier, Date date, boolean callHooks) {
    ...
    if (callHooks) {
      ...
      Thread.startVirtualThread(() -> {
        var delay = 1;
        var random = RandomUtils.secure();
        for (var hook : hooks) {
          log.info("sending email expense to accountant's hook: {}", hook.getExpenseReceiveEmailHook());
          for (var archivedFee : archivedFees) {
            if ("peppol".equalsIgnoreCase(archivedFee.getTag())) {
              // 2025-11-08 07:20 just filter peppol as the accountant receives it
              // automatically from billit
              continue;
            }
            var subject = "%s Expense %s".formatted(personalInfo.getOrganizationName(),
                archivedFee.getSubject());
            var description = archivedFee.getBody();
            var sendingDate = LocalDateTime.now().plusMinutes(delay);
            delay += random.randomInt(1, 5);
            mailJobRepository.sendDelayedMail(List.of(hook.getExpenseReceiveEmailHook()), subject,
                description, false, archivedFee.getAttachmentIds(), sendingDate);
          }
        }
      });
    }
