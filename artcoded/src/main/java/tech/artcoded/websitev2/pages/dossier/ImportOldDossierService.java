...
public class ImportOldDossierService {
    ...
    public void create(MultipartFile zip) {
        ...
        try {
            ...
            try (FileInputStream xlsxFIS = new FileInputStream(xlsx)) {
                ...
                for (var clientRow : clientRows) {
                    BillableClient client = BillableClient.builder().address(clientRow.address)
                            ...
                            .startDate(clientRow.startDate).endDate(clientRow.endDate).imported(true).importedDate(date)
                            .maxDaysToPay(clientRow.maxDaysToPay).city(clientRow.city).rate(clientRow.rate)
                            .name(clientRow.name).build();
                    if (billableClientService.findAll().stream()
                            .noneMatch(cli -> client.getName().equals(cli.getName())
                                    || client.getVatNumber().equals(cli.getVatNumber())
                ...
                for (var expenseRow : expenseRows) {
                    ...
                    for (var file : expenseRow.files) {
                        var mock = MockMultipartFile.builder().name(file.getName()).bytes(readFileToByteArray(file))
                                .contentType(guessContentTypeFromName(file.getName())).originalFilename(file.getName())
                                .build();
                    ...
                    expenseDossier.add(feeService.findById(expense.getId())
                            ...
                            .orElseThrow(
                                    () -> new RuntimeException("exepense '%s' not found".formatted(expense.getId()))));
                ...
                for (var invoiceRow : invoiceRows) {
                    ...
                    InvoiceGeneration invoiceToSave = InvoiceGeneration.builder()
                            .dateOfInvoice(invoiceRow.dateOfInvoice).invoiceNumber(invoiceRow.number)
                            .seqInvoiceNumber(-1L) // since it's an old invoice number, don't mess up with the sequence
                            .invoiceTable(List.of(tech.artcoded.websitev2.pages.invoice.InvoiceRow.builder()
                                    .amount(invoiceRow.amount).amountType(client.rateType).rateType(client.rateType)
                                    .nature(invoiceRow.nature).period(invoiceRow.period).rate(invoiceRow.client.rate)
                                    .projectName(client.projectName).build()))
                            .dateCreation(invoiceRow.dateOfInvoice).taxRate(invoiceRow.taxRate)
                            .maxDaysToPay(client.maxDaysToPay).uploadedManually(true).skipPeppol(true)
                    ...
                    invoiceService.manualUpload(
                            MockMultipartFile.builder().name(invoiceRow.file.getName())
                    ...
                    invoiceDossier.add(invoiceService.findById(invoiceGeneration.getId())
                            .map(invoice -> invoiceService
                                    .update(invoice.toBuilder().imported(true).importedDate(date).build()))
                ...
                for (var dossierRow : dossierRows) {
                    Dossier dossier = dossierService.save(
                            Dossier.builder().description(dossierRow.description).comment("dossier imported from csv")
            ...
            notificationService.sendEvent(
                    "Failed to create dossier(s) from %s! Check the logs".formatted(zip.getName()),
    ...
    private List<ClientRow> extractClients(Workbook workbook) throws ParseException {
        ...
        while (iterator.hasNext()) {
            ...
            clientRows.add(new ClientRow(name, projectName, email, phone, vat, address, city, parseDate(startDate),
                    parseDate(endDate), Integer.valueOf(maxDaysToPay), new BigDecimal(rate),
    ...
    private Date parseDate(String s) {
        ...
        return new Date(parse(s, ofPattern("dd/MM/yyyy")).atTime(11, 0, 0).toInstant(ZoneOffset.ofHoursMinutes(11, 0))
                .toEpochMilli());
    ...
    private List<InvoiceRow> extractInvoices(Workbook workbook, List<DossierRow> dossierRows,
            List<ClientRow> clientRows, File directory) throws ParseException {
        ...
        while (iterator.hasNext()) {
            ...
            ClientRow clientRow = clientRows.stream()
                    .filter(client -> client.name().equals(DATA_FORMATTER.formatCellValue(row.getCell(5)).trim()))
            ...
            DossierRow dossierRow = dossierRows.stream()
                    .filter(dossier -> dossier.name().equals(DATA_FORMATTER.formatCellValue(row.getCell(7)).trim()))
                    .findFirst().orElseThrow(() -> new RuntimeException("Dossier not found!"));
            ...
            invoiceRows.add(new InvoiceRow(number, nature, period, parseDate(dateOfInvoice), taxRate, clientRow, amount,
                    dossierRow, file));
    ...
    private List<ExpenseRow> extractExpenses(Workbook workbook, List<DossierRow> dossierRows, File directory)
        ...
        while (iterator.hasNext()) {
            ...
            DossierRow dossierRow = dossierRows.stream()
                    .filter(dossier -> dossier.name().equals(DATA_FORMATTER.formatCellValue(row.getCell(4)).trim()))
                    .findFirst().orElseThrow(() -> new RuntimeException("Dossier not found!"));

            String fileNames = DATA_FORMATTER.formatCellValue(row.getCell(5)).trim();
            var files = Arrays.stream(fileNames.split(",")).filter(StringUtils::isNotEmpty)
            ...
            expenseRows.add(
                    new ExpenseRow(title, description, parseDate(receivedDate), label, dossierRow, files, hvat, vat));
    ...
    record ClientRow(String name, String projectName, String email, String phone, String vat, String address,
            String city, Date startDate, Date endDate, Integer maxDaysToPay, BigDecimal rate, RateType rateType) {
    }
    ...
    record ExpenseRow(String title, String description, Date receivedDate, String label, DossierRow dossier,
            List<File> files, BigDecimal hvat, BigDecimal vat) {
    }
