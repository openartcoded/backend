...
public class InvoiceGeneration implements Serializable {
    ...
    public static String generateInvoiceNumber() {
        return DateTimeFormatter.ofPattern("MMyyyy") // this is now just a reference
                .format(LocalDate.now()).concat("-").concat(RandomStringUtils.randomAlphabetic(2).toUpperCase());
    }
    ...
    public static String generateStructuredReference(InvoiceGeneration i) {
        ...
        return "+++" + fullReference.substring(0, 3) + "/" + fullReference.substring(3, 7) + "/"
    ...
    public String getNewInvoiceNumber() {
        ...
        if (dateOfInvoice.isAfter(FORMAT_INVOICE_NUMBER_FROM_APRIL_2024)) {
            return this.getInvoiceTable().stream().findFirst().map(p -> p.getPeriod()).filter(Objects::nonNull)
                    .flatMap(p -> Arrays.stream(p.split("/")).skip(1).findFirst()).filter(StringUtils::isNotEmpty)
            ...
            return this.getInvoiceTable().stream().findFirst().map(p -> p.getPeriod()).filter(Objects::nonNull)
                    .map(p -> p.replace("/", "")).filter(StringUtils::isNotEmpty).map(p -> p.concat("-")).orElse("")
    ...
    public BigDecimal getSubTotal() {
        return invoiceTable.stream().map(InvoiceRow::getTotal).reduce(new BigDecimal(0), BigDecimal::add).setScale(2,
                RoundingMode.DOWN);
    }
    ...
    public Date getDueDate() {
        ...
        return Date.from(this.dateOfInvoice.toInstant().atZone(zone).toLocalDate().plusDays(maxDaysToPay)
    ...
    public BigDecimal getTaxes() {
        return this.getSubTotal().multiply(taxRate.divide(new BigDecimal("100"), 2, RoundingMode.UNNECESSARY))
