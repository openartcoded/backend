...
public class LabelService {
  ...
  private Label save(Label label) {
    ...
    checkArgument(isNotEmpty(colorHex) && isValid(colorHex),
        "Color must be valid!");
    ...
    if (byColorHex.isPresent() &&
        !byColorHex.get().getName().equalsIgnoreCase(name)) {
      throw new RuntimeException("Cannot have the same label twice!");
    }
    var toUpdate = byNameIgnoreCase.map(Label::toBuilder)
        .orElseGet(label::toBuilder)
        .name(name)
        .colorHex(colorHex)
