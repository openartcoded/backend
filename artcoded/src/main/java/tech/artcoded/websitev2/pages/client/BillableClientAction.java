...
public class BillableClientAction implements Action {
  ...
  public ActionResult run(List<ActionParameter> parameters) {
    ...
    try {
      List<BillableClient> canStart = repository.findByContractStatusAndStartDateIsAfter(ContractStatus.NOT_STARTED_YET, date);
      ...
      repository.saveAll(canStart.stream().map(s -> s.toBuilder().contractStatus(ContractStatus.ONGOING).build())
        ...
        .toList());
      List<BillableClient> canEnd = repository.findByContractStatusInAndEndDateDateIsBefore(List.of(
  ...
  public static ActionMetadata getDefaultMetadata() {
    return ActionMetadata.builder()
      .key(ACTION_KEY)
      .title("Billable Client contract status update")
      .description("An action to check periodically either if the contract status should be set to ongoing/ended")
