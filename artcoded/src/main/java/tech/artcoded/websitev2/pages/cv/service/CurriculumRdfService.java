...
public class CurriculumRdfService {
    ...
    public CurriculumRdfService(ProducerTemplate producerTemplate, SparqlQueryStore queryStore,
    ...
    public void pushTriples(String curriculumId) {
        Curriculum cv = curriculumRepository.findById(curriculumId)
                .orElseThrow(() -> new RuntimeException("no cv found"));
        var computedQuery = queryStore.getQueryWithParameters("cvRdf", Map.of("graph", defaultGraph, "cv", cv));
