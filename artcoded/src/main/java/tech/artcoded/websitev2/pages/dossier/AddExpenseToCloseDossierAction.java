...
public class AddExpenseToCloseDossierAction implements Action {
  ...
  public ActionResult run(List<ActionParameter> parameters) {
    ...
    try {
      ...
      var dossierId = parameters.stream()
          .filter(p -> PARAMETER_DOSSIER_ID.equals(p.getKey()))
      ...
      var expenseIds = parameters.stream()
          .filter(p -> PARAMETER_FEE_IDS.equals(p.getKey()))
          .map(ActionParameter::getValue)
          .filter(p -> StringUtils.isNotEmpty(p)).findFirst()
