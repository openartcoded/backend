...
public class TimesheetAction implements Action {
    ...
    public ActionResult run(List<ActionParameter> parameters) {
        ...
        try {
            ...
            datesBetween.stream().collect(Collectors.groupingBy(YearMonth::from)).forEach((yearMonth, localDates) -> {
    ...
    public PeriodType getPeriodTypeByDay(BillableClient client, Date date) {
        ...
        return switch (dayOfWeek) {
            ...
            default -> client.getDefaultWorkingDays().contains(dayOfWeek) ? PeriodType.WORKING_DAY
                    : PeriodType.AUTHORIZED_HOLIDAYS;
        };
    }
