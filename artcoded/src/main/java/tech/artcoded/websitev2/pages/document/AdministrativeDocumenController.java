...
public class AdministrativeDocumenController {
    ...
    public ResponseEntity<AdministrativeDocument> toggleBookmarked(@RequestParam("id") String id) {
        return service.toggleBookmarked(id).map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<Void> save(@RequestParam("title") String title,
