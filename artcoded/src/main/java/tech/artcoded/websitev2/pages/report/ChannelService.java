...
public class ChannelService {
  ...
  public void checkUnreadMessages() {
    ...
    var oneHourAgo = DateHelper.toDate(LocalDateTime.now().minusHours(1));

    channelRepository.findAll().forEach(channel -> {
      log.info("sending channel message email...");
      Map<String, List<Channel.Message>> messages = channel.getMessages().stream()
          .filter(msg -> !msg.read() && (channel.getNotifyDate() == null || channel.getNotifyDate().before(oneHourAgo))
      ...
      if (!messages.isEmpty()) {
        ...
        for (var e : messages.entrySet()) {
          mailJobRepository
              .sendDelayedMail(channel.getSubscribers().stream().filter(s -> !e.getKey().equals(s)).toList(),
