...
public class RdfUtilsRestController {
    ...
    public ResponseEntity<ByteArrayResource> fileToLang(@RequestPart("file") MultipartFile file,
            @RequestParam("lang") String lang) {
        try {
            String modelConverted = ModelConverter.inputStreamToLang(file.getOriginalFilename(), file::getInputStream,
                    lang);
            return RestUtil.transformToByteArrayResource(
                    "file_".concat(System.currentTimeMillis() + "").concat(".")
                            .concat(ModelConverter.getExtension(lang)),
                    ModelConverter.getContentType(lang), modelConverted.getBytes());
        } catch (Exception e) {
            String stackTrace = ExceptionUtils.getStackTrace(e);
            return RestUtil.transformToByteArrayResource(
                    "error_".concat(System.currentTimeMillis() + "").concat(".txt"), MediaType.TEXT_PLAIN_VALUE,
    ...
    public ResponseEntity<String> fileToLang(@RequestPart("modelFile") MultipartFile modelFile,
        ...
        try {
            ...
            Optional<String> validate = ShaclValidationUtils.validate(modelFile.getInputStream(),
                    ModelConverter.filenameToLang(modelFile.getOriginalFilename()), shaclFile.getInputStream(),
                    ModelConverter.filenameToLang(shaclFile.getOriginalFilename()));
            return validate.map(ResponseEntity.badRequest()::body)
                    .orElseGet(ResponseEntity.ok().headers(headers)::build);
        } catch (Exception e) {
    ...
    public ResponseEntity<String> modelConversion(@RequestParam("model") String model,
            @RequestParam("langOfModel") String langOfModel, @RequestParam("lang") String lang) {
        try {
    ...
    public List<String> getAllowedExtensions() {
        return ModelConverter.getAllowedLanguages().stream().map(ModelConverter::getExtension)
