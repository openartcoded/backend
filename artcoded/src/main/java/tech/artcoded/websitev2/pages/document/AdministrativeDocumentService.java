...
public class AdministrativeDocumentService implements ILinkable {
    ...
    public void delete(String id) {
        ...
        repository.findById(id).filter(doc -> !doc.isLocked()).ifPresentOrElse(doc -> {
            ...
            exposedEventService.sendEvent(AdministrativeDocumentRemoved.builder().documentId(id).build());
            this.notificationService.sendEvent("document with id %s deleted".formatted(doc.getId()),
                    ADMINISTRATIVE_DOCUMENT_DELETED, doc.getId());
            ...
            this.notificationService.sendEvent(
                    ...
                    ADMINISTRATIVE_DOCUMENT_DELETED, id);
    ...
    public Page<AdministrativeDocument> search(AdministrativeDocumentSearchCriteria searchCriteria, Pageable pageable) {
        ...
        if (StringUtils.isNotEmpty(searchCriteria.getDescription())) {
            criteriaList
                    .add(Criteria.where("description").regex(".*%s.*".formatted(searchCriteria.getDescription()), "i"));
        }
    ...
    public void save(String id, String title, String description, List<String> tags, MultipartFile doc) {
        AdministrativeDocument ad = ofNullable(id).filter(StringUtils::isNotEmpty).flatMap(this.repository::findById)
                .orElseGet(() -> AdministrativeDocument.builder().id(IdGenerators.get()).build());
        String documentUploadId = this.uploadAndDeleteIfExist(doc, ad.getId(), ad.getAttachmentId());
        log.info("saving document to mongo...");

        AdministrativeDocument save = repository
                .save(ad.toBuilder().title(title).description(description).updatedDate(new Date())
                        .attachmentId(documentUploadId).tags(ofNullable(tags).orElseGet(List::of)).build());

        exposedEventService.sendEvent(AdministrativeDocumentAddedOrUpdated.builder().documentId(save.getId())
                .title(save.getTitle()).description(save.getDescription()).uploadId(save.getAttachmentId()).build());
    ...
    public Optional<AdministrativeDocument> toggleBookmarked(String id) {
        return repository.findById(id).map(d -> repository.save(d.toBuilder().updatedDate(new Date())
                .bookmarked(!d.isBookmarked()).bookmarkedDate(d.isBookmarked() ? null : new Date()).build()));
    }

    private String uploadAndDeleteIfExist(MultipartFile file, String id, String uploadId) {
        String newUploadId;
        Optional<String> optionalUploadId = ofNullable(file).filter(Predicate.not(MultipartFile::isEmpty))
    ...
    public String getCorrelationLabel(String correlationId) {
        return this.findById(correlationId).map(d -> this.toLabel(d)).orElse(null);
    }
    ...
    public Map<String, String> getCorrelationLabels(Collection<String> correlationIds) {
        return this.findAll(correlationIds).stream().map(doc -> Map.entry(doc.getId(), this.toLabel(doc)))
