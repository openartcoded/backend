...
public class TimesheetPeriod {
    ...
    public long getDuration() {
        Long durationMorning = ofNullable(morningStartTime).map(DateHelper::toLocalDateTime)
                .map(localDateTime -> Duration
                        .between(localDateTime,
                                ofNullable(morningEndTime).map(DateHelper::toLocalDateTime).orElse(localDateTime))
                        .toMinutes())
                .orElse(0L);

        Long durationAfternoon = ofNullable(afternoonStartTime).map(DateHelper::toLocalDateTime)
                .map(localDateTime -> Duration
                        .between(localDateTime,
                                ofNullable(afternoonEndTime).map(DateHelper::toLocalDateTime).orElse(localDateTime))
                        .toMinutes())
                .orElse(0L);
    ...
    public boolean isRowFilled() {
        return afternoonEndTime != null && afternoonStartTime != null && morningStartTime != null
                && morningEndTime != null;
    }
}
