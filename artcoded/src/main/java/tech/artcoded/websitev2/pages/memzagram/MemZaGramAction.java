    ...
    public ActionResult run(List<ActionParameter> parameters) {
        ...
        try {
            repository.findByVisibleIsFalseAndDateOfVisibilityIsBefore(date).stream()
                    ...
                    .map(memz -> memz.toBuilder()
                            .imageUploadId(
                                    fileUploadService.updateVisibility(memz.getImageUploadId(), memz.getId(), true)
                                            .orElseThrow(() -> new RuntimeException("could not find image!")))
                            .thumbnailUploadId(
                                    fileUploadService.updateVisibility(memz.getThumbnailUploadId(), memz.getId(), true)
                                            .orElseThrow(() -> new RuntimeException("could not find thumbnail!")))
                            .updatedDate(new Date()).visible(true).build())
                    .map(repository::save)
                    .forEach(visibleMemz -> notificationService.sendEvent(
                            "Memz %s set to visible".formatted(visibleMemz.getTitle()), MEMZ_SET_VISIBLE,
                            visibleMemz.getId()));
