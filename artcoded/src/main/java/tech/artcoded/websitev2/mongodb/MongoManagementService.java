...
public class MongoManagementService {
    ...
    private final LegacyFileUploadService fileUploadService;
    ...
    public MongoManagementService(BuildProperties buildProperties, Environment environment, MongoTemplate mongoTemplate,
            CacheManager cacheManager, NotificationService notificationService,
            LegacyFileUploadService fileUploadService, Configuration configuration) {
    ...
    private File getDumpFolder(boolean snapshot) {
        String pathToDumpFolder = environment.getRequiredProperty(
                snapshot ? "application.mongo.pathToDumpSnapshot" : "application.mongo.pathToDump");
    ...
    public List<String> restore(String archiveName, String to, boolean snapshot) throws Exception {
        ...
        try {
            ...
            // clear all caches
            cacheManager.getCacheNames().forEach(c -> ofNullable(c).filter(StringUtils::isNotEmpty)
                    .map(cacheManager::getCache).ifPresent(Cache::clear));
            ...
            switch (FilenameUtils.getExtension(archive.getName())) {
                ...
                default -> throw new RuntimeException(
                        "format for file %s not valid (zip or tar.gz)".formatted(archive.getName()));
            }
            ...
            var from = ofNullable(fromDirectory.listFiles()).stream().findFirst().filter(d -> d.length == 1)
                    .map(d -> d[0]).filter(File::isDirectory).map(File::getName)
                    .orElseThrow(() -> new RuntimeException("Could not determine from where to restore"));

            Map<String, String> templateVariables = Map.of("username",
                    environment.getRequiredProperty("spring.data.mongodb.username"), "password",
                    environment.getRequiredProperty("spring.data.mongodb.password"), "adminDatabase",
                    environment.getRequiredProperty("spring.data.mongodb.authentication-database"), "host",
                    environment.getRequiredProperty("spring.data.mongodb.host"), "port",
                    environment.getRequiredProperty("spring.data.mongodb.port"), "from", from, "to", to);
            ...
            ProcessResult result = new ProcessExecutor().readOutput(true).commandSplit(restoreScript).directory(unzip)
            ...
            this.notificationService.sendEvent(
                    "Restore db from '%s' to '%s' with archive : %s".formatted(from, to, archiveName),
                    NOTIFICATION_TYPE_RESTORE, IdGenerators.get());
    ...
    public List<String> dump(boolean snapshot) {
        try {
            ...
            Map<String, String> templateVariables = Map.of("username",
                    ...
                    environment.getRequiredProperty("spring.data.mongodb.authentication-database"), "host",
                    environment.getRequiredProperty("spring.data.mongodb.host"), "port",
                    environment.getRequiredProperty("spring.data.mongodb.port"), "dbName",
                    environment.getRequiredProperty("spring.data.mongodb.database"));
            ...
            ProcessResult result = new ProcessExecutor().readOutput(true).commandSplit(dumpScript).directory(folder)
                    .redirectOutput(Slf4jStream.of(log).asInfo()).execute();
            ...
            var archiveFileName = archiveName.concat(".tar.gz");
            File tarGzFile = new File(toDeleteDirectory, archiveFileName);
            CompressionHelper.tar(tarGzFile, snapshot ? List.of(new SourceType(folder, true))
                    : List.of(new SourceType(folder, true), new SourceType(uploadFolder, false)));
            ...
            log.info("Added file {} to {}", archiveFileName, dumpFolder.getAbsolutePath());

            if (!snapshot) {
                this.notificationService.sendEvent("New Dump: %s".formatted(archiveName.concat(".tar.gz")),
                        NOTIFICATION_TYPE_DUMP, IdGenerators.get());
            } else {
