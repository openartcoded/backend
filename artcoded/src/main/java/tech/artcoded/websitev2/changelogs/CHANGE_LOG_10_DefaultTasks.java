...
@ChangeUnit(id = "default-tasks", order = "10", author = "Nordine Bittich")
public class CHANGE_LOG_10_DefaultTasks {
    ...
    public void execute(ActionService actionService, ReminderTaskService taskService) throws IOException {
        if (taskService.findAll().isEmpty()) {
            ...
            Consumer<SaveAction> saveTask = sa -> allowedActions.stream()
                    .filter(actionMetadata -> sa.actionKey().equals(actionMetadata.getKey())).findFirst()
                    .ifPresent(am -> {
                        taskService.save(ReminderTask.builder().actionKey(am.getKey()).actionParameters(List.of())
                                .cronExpression(sa.cronExpression()).title(am.getTitle()).inAppNotification(false)
                                .description(am.getDescription()).build(), false);
                    });
