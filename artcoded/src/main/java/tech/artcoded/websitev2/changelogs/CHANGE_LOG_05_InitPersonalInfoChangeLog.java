...
public class CHANGE_LOG_05_InitPersonalInfoChangeLog {
    ...
    public void execute(PersonalInfoService personalInfoService, PersonalInfoRepository repository) {
        ...
        var defaultInitials = new ClassPathResource("misc/initials.png");
        repository.findAll().stream().findFirst()
                .ifPresentOrElse(personalInfo -> log.info("Current personal info {}", personalInfo), () -> {
                    personalInfoService.save(PersonalInfo.builder().ceoFullName("Bittich Nordine").note(
                            ...
                            .organizationAddress("Bekkerstreet 83").organizationCity("London")
                            .organizationName("MYCOMPANY SPRL").organizationBankAccount("BE88 0099 4444 7777")
                            .organizationEmailAddress("contact@woops.com").organizationPostCode("1000").maxDaysToPay(30)
                            .organizationPhoneNumber("0485.55.55.55").vatNumber("BE 0555.555.555")
                            .organizationBankBIC("GEBABEBX").financeCharge(new BigDecimal("1.5")).build(),
                            MockMultipartFile.builder().contentType(MediaType.IMAGE_PNG_VALUE).name("default-logo.png")
                                    .bytes(toSupplier(() -> toByteArray(defaultLogo.getInputStream())).get())
                                    .originalFilename("default-logo.png").build(),
                            MockMultipartFile.builder().contentType(MediaType.IMAGE_PNG_VALUE)
                                    .name("default-signature.png")
                                    .bytes(toSupplier(() -> toByteArray(defaultSignature.getInputStream())).get())
                                    .originalFilename("default-signature.png").build(),

                            MockMultipartFile.builder().contentType(MediaType.IMAGE_PNG_VALUE)
                                    .name("default-initials.png")
                                    .bytes(toSupplier(() -> toByteArray(defaultInitials.getInputStream())).get())
                                    .originalFilename("default-initials.png").build());
                });
    }
}
