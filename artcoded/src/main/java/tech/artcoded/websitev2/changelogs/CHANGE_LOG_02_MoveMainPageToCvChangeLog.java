...
@ChangeUnit(id = "move-main-page-to-cv", order = "2", author = "Nordine Bittich")
public class CHANGE_LOG_02_MoveMainPageToCvChangeLog {
    ...
    public void execution(CurriculumRepository curriculumRepository, MongoTemplate mongoTemplate) {
        if (mongoTemplate.collectionExists("mainPage")) {
            ...
            curriculumRepository.findAll().stream().findFirst()
                    .ifPresent(curriculum -> curriculumRepository.save(curriculum.toBuilder().updatedDate(new Date())
                            .introduction(
                                    """
