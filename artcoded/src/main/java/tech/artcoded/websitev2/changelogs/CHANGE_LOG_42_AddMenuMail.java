...
public class CHANGE_LOG_42_AddMenuMail {
    ...
    public void execute(MenuLinkRepository menuLinkRepository) throws IOException {
        ...
        menuLinkRepository.findByOrderByOrderDesc().stream().findFirst().ifPresent(m -> {
            ...
            menuLinkRepository.save(MenuLink.builder().description("Mails").icon(new String[] { "fas", "mail-bulk" })
                    .routerLink(new String[] { "mails" }).title("Mails").order(m.getOrder() + 1).updatedDate(new Date())
