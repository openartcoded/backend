...
public class CHANGE_LOG_28_AddInvoiceTemplateEnd2022 {
    ...
    public void execute(InvoiceService service) throws IOException {
        ...
        try (var is = templateRes.getInputStream()) {
            service.addTemplate("Template End 2022",
                    MockMultipartFile.builder().contentType("application/octet-stream").name("template-end-2022.ftl")
                            .originalFilename("template-end-2022.ftl").bytes(IOUtils.toByteArray(is)).build());
        }
