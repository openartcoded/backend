...
@Slf4j
@ChangeUnit(id = "add-menu-billable-client", order = "16", author = "Nordine Bittich")
public class CHANGE_LOG_16_AddMenuBillableClient {
    ...
    public void execute(MenuLinkRepository menuLinkRepository) throws IOException {
        menuLinkRepository.findByOrderByOrderDesc().stream().findFirst().ifPresent(m -> {
            ...
            menuLinkRepository.save(MenuLink.builder().description("Billable clients")
                    .icon(new String[] { "fas", "user" }).routerLink(new String[] { "billable-clients" })
                    .title("Clients").order(m.getOrder() + 1).updatedDate(new Date())
                    .routerLinkActiveOptions(RouterLinkOption.builder().exact(true).build()).build());
        });
