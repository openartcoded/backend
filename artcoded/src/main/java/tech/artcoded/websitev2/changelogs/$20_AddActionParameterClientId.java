...
@Slf4j
@ChangeUnit(id = "add-action-parameter-client-id",
  order = "20",
  author = "Nordine Bittich")
public class $20_AddActionParameterClientId {
  ...
  public void execute(ReminderTaskRepository taskRepository) throws IOException {
    taskRepository.findByActionKeyIsNotNull().stream()
      .filter(t -> ACTION_KEY.equals(t.getActionKey()) && t.getActionParameters().stream().noneMatch(p -> PARAMETER_CLIENT_ID.equals(p.getKey())))
