...
@Slf4j
@ChangeUnit(id = "rewrite-file-upload", order = "24", author = "Nordine Bittich")
public class CHANGE_LOG_24_RewriteFileUpload {
    ...
    public void execute(MongoDatabaseFactory mongoDatabaseFactory, MappingMongoConverter mappingMongoConverter,
        ...
        for (GridFSFile gridFSFile : gridFsFiles) {
            var metadata = mapper.readValue(ofNullable(gridFSFile.getMetadata()).map(Document::toJson).orElse("{}"),
            ...
            try (var is = uploadToInputStream(gridFsTemplate, gridFSFile)) {
                String fileName = normalize(
                        RegExUtils.replaceAll(stripAccents(metadata.getOriginalFilename()), "[^a-zA-Z0-9\\.\\-]", "_"));
                FileUpload upload = FileUpload.builder().id(id).correlationId(metadata.correlationId)
                        ...
                        .contentType(
                                ofNullable(metadata.contentType).orElseGet(() -> guessContentTypeFromName(fileName)))
                        .name(metadata.name).size(metadata.size).originalFilename(fileName)
                        .publicResource(metadata.publicResource).build();
