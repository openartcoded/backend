...
public class SmsAction implements Action {
  ...
  public static ActionMetadata getDefaultMetadata() {
    return ActionMetadata.builder()
        .key(ACTION_KEY)
        .title("Sms Action")
        .description("An action to send an sms")
        .allowedParameters(List.of(ActionParameter.builder()
            .key(PARAMETER_PHONE_NUMBER)
            .parameterType(ActionParameterType.STRING)
            .required(false)
            .description("The phone number (e.g +32/487.11.11.11). Only these characters are allowed")
            .build(),
            ActionParameter.builder()
                .key(PARAMETER_MESSAGE)
                .parameterType(ActionParameterType.STRING)
                .required(false)
  ...
  public ActionResult run(List<ActionParameter> parameters) {
    ...
    try {
      ...
      var phoneNumber = parameters.stream()
          .filter(p -> PARAMETER_PHONE_NUMBER.equals(p.getKey()))
          .map(ActionParameter::getValue)
          .filter(p -> StringUtils.isNotEmpty(p)).findFirst()
          .orElseThrow(() -> new RuntimeException("phone number missing"));
      messages.add("phone number: " + phoneNumber);

      var message = parameters.stream()
          .filter(p -> PARAMETER_MESSAGE.equals(p.getKey()))
          .map(ActionParameter::getValue)
          .filter(p -> StringUtils.isNotEmpty(p)).findFirst()
